...
{
    $id=$_POST['username'];
    $name=mysqli_real_escape_string($dbhandle, $_POST['profilename']);
    $role=mysqli_real_escape_string($dbhandle, $_POST['role']);

    mysqli_query($dbhandle,"UPDATE schedule_staff SET name='$name', role='$role' WHERE username='$id'");
...
$id = $_GET['id'];
$result = mysqli_query($dbhandle,"SELECT stock_name, stock_amount, cost FROM inventory WHERE S=".$_GET['id']);
